  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Bundesliga verilerini al
        setTakimlar(data.bundesliga || []);
        setLoading(false);
      } catch (err) {
        console.error('Veri çekme hatası:', err);
        setError('Bundesliga verileri yüklenirken bir hata oluştu');
        setLoading(false);
      }
    };
...
export default BundesligaTablosu; 